...
@section('title', 'Edit User Privileges - ' . $user->name)

@section('content')
<div class="max-w-4xl mx-auto space-y-6" x-data="userEditForm()">
    <!-- Top Navigation -->
    <div class="flex items-center justify-between">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h1 class="text-2xl font-black text-slate-900 tracking-tight">Edit Privileges: {{ $user->name }}</h1>
                <p class="text-xs text-slate-500">Update system role, reset password, and modify module function authorizations.</p>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $user->name) }}" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm font-medium">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email', $user->email) }}" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm font-medium">
                    @error('email')<p class="text-xs text-rose-500 mt-1 font-semibold">{{ $message }}</p>@enderror
                </div>

                <!-- Optional Password Reset -->
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-2">New Password <span class="text-slate-400 font-normal">(Leave blank to keep current)</span></label>
                    <input type="password" name="password" placeholder="Enter new password to reset" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm font-medium">
                ...
                <div>
                    ...
                    <input type="password" name="password_confirmation" placeholder="Repeat new password" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm font-medium">
                </div>
                ...
                <div>
                    ...
                    <select name="role" x-model="selectedRole" @change="applyRolePreset(selectedRole)" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm font-semibold text-slate-800">
                        ...
                        <option value="{{ $key }}" {{ old('role', $user->role) === $key ? 'selected' : '' }}>{{ $role['name'] }}</option>
                        @endforeach
                    </select>
                </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8 space-y-6">
            ...
            <p class="text-xs text-slate-500">Customize the specific modules this user can view, edit, or administer.</p>

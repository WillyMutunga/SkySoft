...
<div class="space-y-8">
    ...
    <!-- Welcome Banner with User Role & Privileges -->
    <div class="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 rounded-2xl p-6 sm:p-8 text-white border border-slate-700 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div class="space-y-2">
            <div class="flex items-center gap-2.5">
                <span class="px-3 py-1 rounded-full text-xs font-bold border {{ Auth::user()->role_badge_class }}">
                    {{ Auth::user()->role_title }}
                </span>
                <span class="text-xs text-slate-400">&bull; Logged in as {{ Auth::user()->email }}</span>
            </div>
            <h2 class="text-2xl font-black text-white">Welcome back, {{ Auth::user()->name }}!</h2>
            <p class="text-xs text-slate-300 max-w-xl">
                You have active access to: 
                @if(Auth::user()->isSuperAdmin())
                <span class="text-emerald-400 font-bold">All Platform Features (Super Admin)</span>
                @else
                    @php 
                    $permNames = [];
                    ...
                    @endphp
                    <span class="text-emerald-400 font-bold">{{ implode(', ', $permNames) }}</span>
                @endif
            </p>
        </div>
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.users.index') }}" class="px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-white text-xs font-bold border border-slate-600 transition flex items-center">
                <svg class="w-4 h-4 mr-2 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                <span>Manage Privileges</span>
            </a>
            <a href="{{ route('admin.users.create') }}" class="px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold transition flex items-center shadow-md">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
            <div>
                <p class="text-xs font-bold text-slate-500 uppercase tracking-wider">Staff Users</p>
                <h3 class="text-3xl font-extrabold text-purple-600 mt-1">{{ $stats['total_users'] }}</h3>
                <span class="text-[11px] text-purple-600 font-semibold mt-1 block">{{ $stats['active_users'] }} Active System Accounts</span>
            </div>

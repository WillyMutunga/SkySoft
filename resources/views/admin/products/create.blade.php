...
@section('header_title', 'Create New Product / Solution')
...
<div class="max-w-4xl bg-white rounded-3xl p-8 border border-slate-200 shadow-sm space-y-6">
    ...
    <div class="flex items-center justify-between border-b border-slate-100 pb-4">
        <div>
            ...
            <p class="text-xs text-slate-500">Enter hardware or software specifications</p>
        </div>
    ...
    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="name" required value="{{ old('name') }}" placeholder="e.g. Smart Touch POS Terminal" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-emerald-500">
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div>
                ...
                <select name="price_type" required class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-emerald-500 bg-white">
                    ...
                    <option value="starting_at" {{ old('price_type') === 'starting_at' ? 'selected' : '' }}>Starting From</option>
                    <option value="custom_quote" {{ old('price_type') === 'custom_quote' ? 'selected' : '' }}>Custom Quote</option>
                </select>
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Price (KES)</label>
                <input type="number" step="0.01" name="price" value="{{ old('price') }}" placeholder="e.g. 45000" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-emerald-500">
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Badge Tag</label>
                <input type="text" name="badge" value="{{ old('badge') }}" placeholder="e.g. Best Seller, Popular" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-emerald-500">
            </div>
        </div>
        ...
        <div class="bg-slate-50 p-5 rounded-2xl border border-slate-200 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">Upload Photo from Computer / Phone</label>
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">OR External Image URL</label>
                    <input type="url" name="image_url" value="{{ old('image_url') }}" placeholder="https://images.unsplash.com/..." class="w-full px-4 py-2 rounded-xl border border-slate-300 text-xs focus:ring-2 focus:ring-emerald-500">
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="short_description" rows="2" required placeholder="Brief 1-2 sentence overview for catalog cards..." class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-emerald-500">{{ old('short_description') }}</textarea>
        </div>

        <div>
            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Full Detailed Description *</label>
            <textarea name="description" rows="5" required placeholder="Comprehensive description of capabilities, target business, and warranty..." class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-emerald-500">{{ old('description') }}</textarea>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Features (1 per line)</label>
                <textarea name="features" rows="4" placeholder="M-Pesa STK Integration&#10;KRA eTIMS Ready&#10;Cloud Sync" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-xs font-mono focus:ring-2 focus:ring-emerald-500">{{ old('features') }}</textarea>
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Specifications (Format: Key: Value)</label>
                <textarea name="specs" rows="4" placeholder="Display: 15.6 Inch Touch&#10;RAM: 8GB&#10;Storage: 128GB SSD" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-xs font-mono focus:ring-2 focus:ring-emerald-500">{{ old('specs') }}</textarea>
            </div>
        </div>
        ...
        <div class="flex items-center space-x-6 pt-2 border-t border-slate-100 text-sm">
            ...
            <label class="flex items-center space-x-2 text-slate-700 cursor-pointer">
                <input type="checkbox" name="in_stock" value="1" {{ old('in_stock', 1) ? 'checked' : '' }} class="w-4 h-4 text-emerald-600 rounded">
        ...
        <div class="pt-4 flex justify-end space-x-3">
            ...
            <button type="submit" class="px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold shadow-md transition">Save & Publish Product</button>
        </div>
    </form>
